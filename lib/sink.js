    ...
    value: function exposeURL() {
      ...
      this.root.querySelector('article').querySelectorAll(this.itemSelector).forEach(function (item) {
        if (item.classList.contains('hyperaudio-effect')) {
          ...
          var start = parseFloat(item.querySelector('span[data-t]').getAttribute('data-t').split(',')[0]);
          var end = parseFloat(item.querySelector('span[data-t]:last-child').getAttribute('data-t').split(',').map(function (v) {
            return parseFloat(v);
