  ...
  _createClass(Player, [{
        ...
        var _loop2 = function _loop2(i) {
          ...
          if (t <= time && time < t + d) {
            ...
            if (!candidates[i].parentElement.classList.contains('hyperaudio-scroll')) {
              ...
              // }
              Array.from(_this2.root.querySelectorAll('p.hyperaudio-scroll')).filter(function (s) {
              ...
              var aRect = _this2.root.querySelector('article').getBoundingClientRect();
              var fold = aRect.height + aRect.y;
              var rect = candidates[i].getBoundingClientRect();
              if (rect.y > fold && !candidates[i].classList.contains('hyperaudio-scroll')) {
                // FIXME
                //  && !this.root.querySelector('.velocity-animating')
                candidates[i].classList.add('hyperaudio-scroll');
                // if (jQuery && jQuery.Velocity) {
                ...
                // }
                Array.from(_this2.root.querySelectorAll('span.hyperaudio-scroll')).filter(function (s) {
    ...
    value: function play() {
      ...
      if (!this.root.querySelector('.hyperaudio-current') && this.root.querySelector('.hyperaudio-transcript')) {
        ...
        var pastWords = this.root.querySelectorAll('.hyperaudio-current.hyperaudio-transcript .hyperaudio-past');
        if (pastWords && pastWords.length > 0) {
          Array.from(pastWords).pop().dispatchEvent(_event);
        } else {
          this.root.querySelector('.hyperaudio-current.hyperaudio-transcript').querySelector('*[data-t]').dispatchEvent(_event);
        }
      } else if (media) {
    ...
    key: 'pause',
    value: function pause(skipCurrent) {
      if (skipCurrent) {
        var media = Array.from(this.root.querySelectorAll('video, audio')).find(function (el) {
          return el.style.display !== 'none';
        });
        ...
        this.root.querySelectorAll('video, audio').forEach(function (media) {
          return media2.pause();
        });
      }
    }
    ...
    value: function createMedia(src, type) {
      ...
      if (this.root.querySelector('.hyperaudio-current.hyperaudio-transcript')) {
        var activeMedia = Array.from(this.root.querySelectorAll('video, audio')).find(function (el) {
          return el.style.display !== 'none';
        });
        if (activeMedia) {
          media.style.display = 'none';
