...
var Player = function () {
  function Player(rootNodeOrSelector) {
    ...
    this.root.querySelector('.hyperaudio-progress').parentElement.addEventListener('click', this.onSeek.bind(this));
  }
  ...
  _createClass(Player, [{
    ...
    value: function setHead(time, src) {
      ...
      var _loop = function _loop(j) {
        ...
        if (time > parseFloat(ti) + parseFloat(di) + trim) return 'continue';
    ...
    value: function onSeek(event) {
      var _event$target$getClie = event.target.getClientRects()[0],
          x = _event$target$getClie.x,
          width = _event$target$getClie.width;
      ...
      // flow-disable-next-line
      targetSection.querySelector('*[data-t]').dispatchEvent(event2);
    }
  }, {
    key: 'progress',
    ...
    value: function getMedia(src, type) {
      ...
      if (media && !media.classList.contains('hyperaudio-enabled')) {
        ...
        media.addEventListener('loadedmetadata', function (event) {
          var item = _this3.root.querySelector('.hyperaudio-transcript[data-src="' + src + '"]');
